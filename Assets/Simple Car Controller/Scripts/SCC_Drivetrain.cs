...
public class SCC_Drivetrain : MonoBehaviour {
    ...
    private void Awake() {
        ...
        //  Set COM immediately so it's correct before the first physics step.
        if (COM)
            Rigid.centerOfMass = COM.localPosition;

        IgnoreBodyWheelCollisions();
        AlignToGround();

    }

    /// <summary>
    /// Prevents body colliders (Box/Mesh) from colliding with WheelColliders on the same vehicle.
    /// </summary>
    private void IgnoreBodyWheelCollisions() {
        ...
        for (int i = 0; i < allColliders.Length; i++) {
            ...
            //  Skip WheelColliders themselves.
            if (allColliders[i] is WheelCollider)
                continue;

            for (int j = 0; j < wheelColliders.Length; j++) {

                Physics.IgnoreCollision(allColliders[i], wheelColliders[j], true);
    ...
    /// <summary>
    /// Aligns the car to the ground on spawn so wheels sit on the surface
    /// instead of the body clipping into the road.
    /// </summary>
    private void AlignToGround() {
        ...
        //  Find the lowest wheel bottom position relative to the car root.
        float lowestWheelBottomWorld = float.MaxValue;
        ...
        for (int i = 0; i < wheels.Length; i++) {
            ...
            WheelCollider wc = wheels[i].wheelCollider.WheelCollider;
            float wheelBottomWorld = wc.transform.position.y - wc.radius;

            if (wheelBottomWorld < lowestWheelBottomWorld)
                lowestWheelBottomWorld = wheelBottomWorld;

        }

        //  Raycast down from the car to find the ground.
        RaycastHit hit;

        if (Physics.Raycast(transform.position + Vector3.up * 2f, Vector3.down, out hit, 50f)) {

            //  Calculate how far the wheel bottoms are from the ground surface.
            float groundY = hit.point.y;
            float offset = lowestWheelBottomWorld - groundY;

            //  Move the car so wheel bottoms sit just above the ground.
            //  Add a small buffer for suspension to settle.
            transform.position += Vector3.up * (-offset + 0.05f);
